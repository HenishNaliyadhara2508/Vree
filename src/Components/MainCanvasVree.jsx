...
const MainCanvasVree = observer(() => {
  ...
  // Handle button clicks to highlight parts
  const handleSelectFrame = () => {
    const loader = new LoaderManager();
    loader.clearOutlines(); // Clear previous outlines
    loader.highlightMesh(vreeStore.frameMesh);
  };

  const handleSelectLenses = () => {
    const loader = new LoaderManager();
    loader.clearOutlines(); // Clear previous outlines
    vreeStore.lensesMesh.forEach((lens) => loader.highlightMesh(lens));
  };

  const handleSelectTemple = () => {
    const loader = new LoaderManager();
    loader.clearOutlines(); // Clear previous outlines
    vreeStore.templeMesh.forEach((temple) => loader.highlightMesh(temple));
  };
